...
class AV(object):
    """
    This class gets a file as input. The methods will query AV cache for already existing results of the file md5,
     and in case not then upload the file to TED, query until an answer with AV results is received,
    ...
    def create_response_info(self, response):
        """
        Create the AV response info of handled file.
    ...
    def check_av_cache(self):
        ...
        print("file md5: {}".format(self.md5))
        data = json.dumps(request)
        print("Sending AV Query request before upload in order to check AV cache")
    ...
    def upload_file(self):
        ...
        print("Sending Upload request for av")
        try:
            ...
            raise
        response_j = response.json()
        print("av Upload response status: {}".format(response_j["response"][0]["status"]["label"]))
        return response_j
    ...
    def query_file(self):
        ...
        """
        print("Start sending Query requests for av after AV upload")
        request = copy.deepcopy(self.request_template)
        request['request'][0]['md5'] = self.md5
        data = json.dumps(request)
        ...
        while (not status_label) or (status_label == "NOT_FOUND"):
            print("Sending Query request for av")
            response = requests.post(url=self.url + "query", data=data, verify=False)
            response_j = response.json()
            status_label = response_j['response'][0]['status']['label']
            if status_label != "NOT_FOUND":
                break
            print("av Query response status still not found")
            time.sleep(SECONDS_TO_WAIT)
            retry_no += 1
            if retry_no == MAX_RETRIES:
                print("Reached query max retries.  Stop waiting for av results")
                break
        return response_j

    def handle_file(self):
        """
        1. Query AV cache for already existing results of the file md5.
            If results exist then goto #4, otherwise- continue to #2
        2. Upload the file to the appliance for av feature.
        3. If upload result is upload_success then query av until receiving results.
        4. Write to file last query/upload response info.
        5. If av result is found malicious then print the resulted av signature.
        """
        ...
        if cache_status_label == "FOUND":
            ...
            if upload_status_label == "UPLOAD_SUCCESS":
                ...
                print("Receiving Query response with av results. status: {}".format(query_status_label))
